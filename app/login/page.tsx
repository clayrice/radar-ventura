...
export default async function Login({searchParams}:{searchParams:Promise<{error?:string;next?:string}>}){
 ...
 return <main id="main" className="login">
  ...
  <p>{partner?'Entre ou crie sua conta para cadastrar sua empresa como parceira.':'Entre ou crie sua conta para personalizar sua edição semanal.'} {process.env.GOOGLE_AUTH_ENABLED==='true'?'Use sua conta Google ou receba um link por email.':'Receba um link seguro por email para continuar.'}</p>
  ...
  <LoginForm next={next} googleEnabled={process.env.GOOGLE_AUTH_ENABLED==='true'}/>
